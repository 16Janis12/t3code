...
import { toCodexCliArgs } from "../../mcp/McpServerResolver.ts";
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        if (mcpSession?.externalServers) {
          const codexExternal = toCodexCliArgs(mcpSession.externalServers);
          codexMcpArgs.push(...codexExternal.args);
          Object.assign(codexMcpEnv, codexExternal.env);
        }
        const hasMcp = codexMcpArgs.length > 0;
